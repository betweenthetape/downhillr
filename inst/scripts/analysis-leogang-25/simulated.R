...
# ------------------------------------------------------------------------------
# Harriet Harnden table showing origin of fastest splits
# ------------------------------------------------------------------------------
table_harriet_harnden <- results |>
  select(name, starts_with("split"), time, event_name, round_type) |>
  bind_rows(timed_training) |>
  filter(name == "Harriet Harnden") |>
  mutate(
    `1` = split_1,
    `2` = split_2 - split_1,
    `3` = split_3 - split_2,
  ...
  tab_header(
    md(
      "**Harriet Harnden's fastest sections were spread across the Finals and Qualifying Round 2**"
    ),
...
gtsave(
  table_harriet_harnden,
  "inst/scripts/analysis-leogang-25/table_harriet_harnden.png",
  zoom = 10
)
...
gtsave(
  ...
  "inst/scripts/analysis-leogang-25/table_possible_faster.png",
  zoom = 10
)
...
table_time_left <- fastest_times_all |>
  ...
  tab_header(
    title = md(
      "**The top five riders with the most time left on the track**"
    ),
    subtitle = md(
...
gtsave(
  table_time_left,
  "inst/scripts/analysis-leogang-25/table_time_left.png",
  zoom = 10
)
...
bump_data <- actual |>
  ...
  mutate(
    color = case_when(
      name == "Veronika Widmann" ~ "#57106e",
      name == "Monika Hrastnik" ~ "#B45834",
      name == "Lisa Bouladou" ~ "#f98e09",
      TRUE ~ "#E7E7E7"
    )
  ) |>
...
plot_bump <- ggplot() +
  ...
  labs(
    ...
    results highlight that the Elite Women ramped up their speed over the race
    weekend, and for the most part, Finals were their fastest runs.
    </span>"
  )

# ggsave(
#   "inst/scripts/analysis-leogang-25/plot_bump.png",
#   plot = plot_bump,
#   width = 2300,
#   height = 3200,
#   units = "px",
...
gtsave(
  table_heat_map,
  "inst/scripts/analysis-leogang-25/table_heat_map.png",
  zoom = 10
)
...
gtsave(
  table_combined_run,
  "inst/scripts/analysis-leogang-25/table_combined_run.png",
  zoom = 10
)
...
ggsave(
  "inst/scripts/analysis-leogang-25/plot_ridges.png",
  plot = plot_ridges,
  width = 2200,
  height = 1800,
  units = "px",
...
# Keep only women who appeared in Finals and find best time for each of these
...
plot_ramp_up <- speed_index |>
  ...
  theme_minimal() +
  labs(
    title = "The best performers were those who had the steepest ramp up in speed over the weekend",
    x = "Final position",
    y = "Rate of change of speed improvment over the race (log scale)"
  ) +
  theme_ridges()

ggsave(
  "inst/scripts/analysis-leogang-25/plot_ramp_up.png",
  plot = plot_ramp_up,
  width = 3500,
  height = 2000,
  units = "px",
