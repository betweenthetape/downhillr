...
#   "inst/scripts/analysis-les-gets-25/table_fastest_splits.png",
#   zoom = 10
# )
...
#   "inst/scripts/analysis-les-gets-25/table_possible_faster.png",
#   zoom = 10
# )
...
# gtsave(
#   table_time_left,
#   "inst/scripts/analysis-les-gets-25/table_time_left.png",
#   zoom = 10
# )
...
plot_bump <- ggplot() +
  ...
  labs(
    ...
    subtitle = "<span> Each riders fastest splits from across the Les Gets
...
# ggsave(
#   "inst/scripts/analysis-les-gets-25/plot_bump.png",
#   plot = plot_bump,
#   width = 2300,
#   height = 3200,
#   units = "px",
...
# gtsave(
#   table_heat_map,
#   "inst/scripts/analysis-les-gets-25/table_heat_map.png",
#   zoom = 10
# )
...
# gtsave(
#   table_combined_run,
#   "inst/scripts/analysis-les-gets-25/table_combined_run.png",
#   zoom = 10
# )
...
# Could Pierron have won if we didn't crash?
dunne_final_time <- fastest_times_final |>
  slice(1) |>
  pull(fastest_time_finals)

pierron_splits_1_3 <- results |>
  filter(name == "Amaury Pierron") |>
  filter(round_type == "Final") |>
  select(split_1, split_2, split_3) |>
  as.numeric()

dunne_pierron_splits_ranked <- fastest_possible_sections |>
  select(-ends_with("_round")) |>
  rename_with(\(x) str_remove_all(x, "_time"), starts_with("section_")) |>
  ...
  add_row(
    name = "Amaury Pierron",
    event_name = "Les Gets",
    section_1 = pierron_splits_1_3[[1]],
    section_2 = pierron_splits_1_3[[2]],
    section_3 = pierron_splits_1_3[[3]],
    section_4 = NA,
    section_5 = NA,
  ) |>
  mutate(
...
table_dunne_pierron <- dunne_pierron_splits_ranked |>
  left_join(image_data) |>
  select(path, name, ends_with("_gap"), ends_with("_rank")) |>
  gt() |>
  text_transform(
  ...
  tab_header(
    title = md("## Could Pierron have beaten Dunne?"),
    subtitle = md(
...
gtsave(
  table_dunne_pierron,
  "inst/scripts/analysis-les-gets-25/table_dunne_pierron.png",
  zoom = 10
)

pierron_fastest_split_4 <- fastest_possible_sections |>
  filter(name == "Amaury Pierron") |>
  pull(section_4_time)

pierron_fastest_split_5 <- fastest_possible_sections |>
  filter(name == "Amaury Pierron") |>
  pull(section_5_time)

# Scenario 1: Actual finals split times from 1-4 + simulated 5th
pierron_simulated_finish <- pierron_splits_1_3[3] +
  pierron_fastest_split_4 +
  pierron_fastest_split_5

print(dunne_final_time)
print(pierron_simulated_finish)
