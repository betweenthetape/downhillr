...
table_jordan_williams <- results |>
  select(name, starts_with("split"), time, event_name, round_type) |>
  bind_rows(timed_training) |>
  filter(name == "Jordan Williams") |>
  mutate(
    `1` = split_1,
    `2` = split_2 - split_1,
    `3` = split_3 - split_2,
  ...
  tab_header(
    md(
      "**Jordan William's fastest sections were spread across the event**"
    ),
    md("Fastest sections (in seconds) are highlighted in green")
  ) |>
  tab_source_note(
    md("_Timed Training runs 3, 4 and 5 were not performed_")
  ) |>
  tab_style(
    style = cell_text(align = "left"),
...
# ggsave(
#   "inst/scripts/analysis-val-di-sole-25/elite-men/plot_bump.png",
#   plot = plot_bump,
#   width = 2300,
#   height = 3200,
#   units = "px",
...
table_combined_run <- fastest_combined_run |>
  ...
  relocate(path) |>
  slice(-2) |>
  mutate(name = if_else(section == 1, "Dylan Maples & Loic Bruni", name)) |>
  gt() |>
  text_transform(
    locations = cells_body(columns = path),
    fn = function(path) {
...
# gtsave(
#   table_combined_run,
#   "inst/scripts/analysis-val-di-sole-25/elite-men/table_combined_run.png",
#   zoom = 10
# )
...
# ggsave(
#   "inst/scripts/analysis-val-di-sole-25/elite-men/plot_ramp_up.png",
#   plot = plot_ramp_up,
#   width = 3500,
#   height = 2000,
#   units = "px",
...
plot_training_qualifying <- speed_index |>
  ...
  theme_ridges() +
  labs(
    title = "There is a loose linear relationship between Elite Men's fastest timed training run and fastest \nqualifying time",
    subtitle = "Runs longer > 250s omitted",
    x = "Fastest timed training run (s)",
    y = "Fastest qualifying time (s)"
  )

# ggsave(
#   "inst/scripts/analysis-val-di-sole-25/elite-men/plot_training_qualifying.png",
...
plot_training_finals <- speed_index |>
  ...
  theme_ridges() +
  labs(
    title = "No relationship is observed between Elite Men's fastest timed training run and fastest \nqualifying time",
    subtitle = "Runs longer > 250s omitted",
    x = "Fastest timed training run (s)",
    y = "Fastest qualifying time (s)"
  )
...
# ggsave(
#   "inst/scripts/analysis-val-di-sole-25/elite-men/plot_training_finals.png",
