...
table_eleonora_farina <- results |>
  select(name, starts_with("split"), time, event_name, round_type) |>
  bind_rows(timed_training) |>
  filter(name == "Eleonora Farina") |>
  mutate(
    `1` = split_1,
    `2` = split_2 - split_1,
    `3` = split_3 - split_2,
  ...
  tab_header(
    md(
      "**Eleonora Farina's fastest sections were spread across the event**"
    ),
  ...
  data_color(
    columns = `2`,
    rows = round_type == "Timed Training 4",
    palette = "#4daf4a"
  ) |>
  data_color(
    columns = `3`,
...
# ggsave(
#   "inst/scripts/analysis-val-di-sole-25/elite-women/plot_bump.png",
#   plot = plot_bump,
#   width = 2300,
#   height = 3200,
#   units = "px",
